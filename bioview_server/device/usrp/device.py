...
from .utils import setup_pps, setup_ref, check_channels, update_usrp_address, get_channel_map
...
# ser = get_usrp_address(self.device_name)
#     if ser is not None:
#         addr = uhd.find(f"serial={ser}")[0]
#     else:
#         addr = uhd.find(self.device_name)[0]
#         ser = addr["serial"]
#         # Store it for future use
#         update_usrp_address(self.device_name, ser)
...
class USRPDevice(Device):
    def __init__(
        ...
        for device_key, device_config in devices.items():
            ...
            self.usrp_tx_streamers[device_key] = None 
            self.usrp_rx_streamers[device_key] = None 
